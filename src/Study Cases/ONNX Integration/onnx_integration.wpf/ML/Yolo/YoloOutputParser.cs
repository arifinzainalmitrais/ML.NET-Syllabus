...
namespace onnx_integration.wpf.ML.Yolo
{
    public class YoloOutputParser
    {
        ...
        private int GetOffset(int x, int y, int channel) => (channel * this.channelStride) + (y * COL_COUNT) + x;
        ...
        private float GetConfidence(float[] modelOutput, int x, int y, int channel) => Sigmoid(modelOutput[GetOffset(x, y, channel + 4)]);
